...
const statusLabel={
  ...
  procesamiento_fallido:'Requiere atención',
...
export default function SaleWorkspace({ config, me, onCompleted }){
  ...
  return <section className="panel sale-panel">
    ...
    <div className={`checkout-strip ${sale&&!editing?'saved-checkout':''}`}><div className="checkout-copy"><strong>{sale?(editing?'Editando venta':'Venta guardada'):'Revisa y guarda la venta'}</strong><span>{sale?(editing?'Guarda los cambios antes de pagar.':status==='entregada'?(config?.ecosystem?.enabled?'Los documentos fueron validados y enviados al cliente.':'Pago aprobado y factura enviada al correo del cliente.'):sale?.pago?.transactionCode?(config?.ecosystem?.enabled?'Pago aprobado. Estamos procesando factura electrónica, Tributación y entrega al cliente.':'Pago aprobado. Estamos generando y enviando tu factura al correo.'):'Tus datos quedan guardados aunque salgas a completar el pago.'):'Primero guardamos esta venta; después podrás cobrarla.'}</span>{sale&&!editing&&<div className="secondary-sale-actions"><button className="link-quiet" disabled={status!=='pendiente_pago'} onClick={()=>setEditing(true)}>Editar venta</button><button className="link-quiet" onClick={reset}>Nueva venta</button></div>}</div>{!sale?<div className="actions"><button className="primary" disabled={busy||totals.total<=0} onClick={create}>Guardar venta</button></div>:editing?<div className="actions"><button className="primary" disabled={busy||totals.total<=0} onClick={create}>Guardar cambios</button><button className="secondary" onClick={()=>setEditing(false)}>Cancelar edición</button></div>:<div className="pay-zone"><span>Total guardado</span><strong>{money(sale.total||totals.total)}</strong><button className="bank-button" disabled={busy||status==='entregada'||Boolean(sale?.pago?.transactionCode)||!config?.bank?.ready} onClick={pay}>{busy?'Procesando...':status==='esperando_banco'?'Continuar pago':'Pagar ahora'}</button></div>}</div>
    {sale?.pago?.transactionCode&&<div className="process-tracker"><div className="process-tracker-head"><div><strong>{config?.ecosystem?.enabled?'Procesamiento de documentos':'Entrega de factura'}</strong><span>{config?.ecosystem?.enabled?'La factura se entrega al correo del cliente cuando todos los servicios terminan correctamente.':'Para esta prueba solo usamos BankyFinanzas y Factura Bonita; los demás servicios quedan desactivados.'}</span></div><span className={`status-chip ${status}`}>{statusLabel[status]||'Procesando'}</span></div><div className="process-steps"><div className="process-step done"><i>✓</i><div><strong>Pago</strong><span>Aprobado por BankyFinanzas</span></div></div>{integrationSteps.map(({key,label,step})=><div className={`process-step ${doneStates.has(step?.estado)?'done':step?.estado==='fallida'||step?.estado==='pendiente_configuracion'?'failed':step?'active':'pending'}`} key={key}><i>{doneStates.has(step?.estado)?'✓':step?.estado==='fallida'||step?.estado==='pendiente_configuracion'?'!':'•'}</i><div><strong>{label}</strong><span>{step?.mensaje||'Pendiente'}</span></div></div>)}</div>{status==='procesamiento_fallido'&&<div className="process-retry"><span>{config?.ecosystem?.enabled?'El pago está seguro; solo falta completar el procesamiento documental.':'El pago está seguro; solo falta completar el envío de la factura al correo.'}</span><button className="secondary" disabled={busy} onClick={retryProcessing}>Reintentar procesamiento</button></div>}</div>}
    ...
    {message&&<div className="alert info">{message}</div>}{sale?.errorDetalle&&sale?.pago?.transactionCode&&<div className="alert error">{sale.errorDetalle}</div>}
