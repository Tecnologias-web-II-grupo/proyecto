...
function FiscalDetails({ factura }) {
  ...
  const bloques = items.map((it, i) => {
    ...
    if (it.baseImponible !== undefined) datos.push(['Base imponible', dinero(it.baseImponible, factura.moneda)]);
    ...
    const extras = [];
    if (comerciales.length) extras.push(`Códigos comerciales: ${comerciales.map((c) => `${c.tipo || '—'}:${c.codigo || '—'}`).join(' · ')}`);
    if (descuentos.length) extras.push(`Descuentos: ${descuentos.map((d) => `${d.codigo || '—'} ${d.naturaleza || ''} ${dinero(d.monto, factura.moneda)}`).join(' · ')}`);
    if (impuestos.length) extras.push(`Impuestos: ${impuestos.map((x) => `${x.codigo || '—'} · ${x.tarifa ?? 0}% · tarifa ${x.codigoTarifaIVA || '—'} · ${dinero(x.monto ?? 0, factura.moneda)}`).join(' | ')}`);
    if (!datos.length && !extras.length) return null;
    return React.createElement('div', { className: 'line-extra', key: `fiscal-${i}` },
      React.createElement('h3', null, `Información complementaria · línea ${it.numeroLinea ?? i + 1}`),
      React.createElement('div', { className: 'grid four' }, ...datos.map(([l, v]) => React.createElement('div', { className: 'datum', key: l }, React.createElement('span', { className: 'label' }, l), React.createElement('strong', null, v)))),
      ...extras.map((x, j) => React.createElement('p', { className: 'small', key: j }, x))
    );
  }).filter(Boolean);
