...
export default function LogoDesigner({ me, onSaved, onBack }){
  ...
  async function save(){
    ...
    try{
      const data=await api('/api/portal/perfil',{method:'PUT',body:fd});
  ...
  return <section className="panel brand-panel">
    ...
    <div className="designer-grid logo-only-grid">
      <div className="logo-controls">
        ...
        <div className="brand-note"><strong>{me?.empresa}</strong><span>Los datos fiscales de tu negocio se toman de la cuenta con la que te registraste.</span></div>
        <button className="primary" onClick={save}>Guardar logo</button>{msg&&<span className="inline-message">{msg}</span>}
      </div>
