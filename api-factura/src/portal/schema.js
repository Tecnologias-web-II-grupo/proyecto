...
async function ensurePortalSchema() {
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS portal_perfiles (
    usuario_id CHAR(36) NOT NULL,nombre_comercial VARCHAR(160) NULL,actividad_economica VARCHAR(12) NULL,telefono VARCHAR(30) NULL,provincia VARCHAR(80) NULL,canton VARCHAR(80) NULL,distrito VARCHAR(80) NULL,otras_senas VARCHAR(255) NULL,
    logo LONGTEXT NULL,logo_blanco LONGTEXT NULL,logo_posicion ENUM('left','center','right') NOT NULL DEFAULT 'left',bank_merchant_id VARCHAR(160) NULL,bank_afiliado BOOLEAN NOT NULL DEFAULT FALSE,created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS portal_clientes (
    id BIGINT NOT NULL AUTO_INCREMENT,usuario_id CHAR(36) NOT NULL,nombre VARCHAR(160) NOT NULL,nombre_comercial VARCHAR(160) NULL,
    tipo_identificacion VARCHAR(2) NOT NULL,numero_identificacion VARBINARY(255) NOT NULL,identificacion_hash CHAR(64) NOT NULL,correo VARCHAR(160) NOT NULL,actividad_economica VARCHAR(12) NULL,
    telefono VARCHAR(30) NULL,provincia VARCHAR(80) NULL,canton VARCHAR(80) NULL,distrito VARCHAR(80) NULL,otras_senas VARCHAR(255) NULL,
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (id),UNIQUE KEY uq_portal_cliente_usuario_identificacion (usuario_id,tipo_identificacion,identificacion_hash),
    KEY idx_portal_clientes_usuario (usuario_id),CONSTRAINT fk_portal_cliente_usuario FOREIGN KEY (usuario_id) REFERENCES portal_usuarios(id) ON DELETE CASCADE ON UPDATE CASCADE
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`);


  await ensureColumn('portal_perfiles','bank_merchant_id','VARCHAR(160) NULL AFTER logo_posicion');
  await ensureColumn('portal_perfiles','bank_afiliado','BOOLEAN NOT NULL DEFAULT FALSE AFTER bank_merchant_id');
  await pool.query(`ALTER TABLE portal_perfiles MODIFY COLUMN provincia VARCHAR(80) NULL, MODIFY COLUMN canton VARCHAR(80) NULL, MODIFY COLUMN distrito VARCHAR(80) NULL`);
  await pool.query(`ALTER TABLE portal_clientes MODIFY COLUMN provincia VARCHAR(80) NULL, MODIFY COLUMN canton VARCHAR(80) NULL, MODIFY COLUMN distrito VARCHAR(80) NULL`);
